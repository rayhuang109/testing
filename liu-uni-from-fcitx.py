...
#with urlopen('https://github.com/fcitx/fcitx-table-extra/raw/master/tables/boshiamy.txt') as f:
#    raw_table = f.read().decode().strip().split('\n')[3:]
with open('boshiamy.txt', 'r') as f:
    raw_table = f.read().strip().split('\n')[3:]

for ent in raw_table:
    key, char = ent.split(" ")
    if len(key) > 4:
        key = key[0:4]
    elif len(key) < 4:
